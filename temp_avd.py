...
for world_id in range(len(WORLDS)):
    ...
    cached_data = read_cached_data(True, dataset_root, targets_file_name=None, output_size=224,
                                   Home_name=current_world.encode())  # encode() convert string to byte
    ...
    detections = np.load('{}/Meta/Detections/{}.npy'.format(dataset_root, current_world),
    ...
    # step 2: localize the point closest to target category point cloud and plan the path
                ...
                rect = patches.Rectangle((x1, y1), x2-x1, y2-y1, linewidth=5,
                                         edgecolor='green', facecolor=(0, 1, 0, 0.5))
                ax.add_patch(rect)
                plt.title('env: {}, target: {}, detection_score: {:.2f}'.format(
                    current_world, target_category, detection_score))
                plt.savefig('{}/env_{}_category_{}_id_{}_left.jpg'.format(saved_folder,
                            current_world, target_category, idx), bbox_inches='tight')
                plt.close()
    ...
    # ==========================================================================================================
                ...
                plt.grid()
                plt.axis('scaled')
                plt.title('env: {}, target: {}, steps: {}, success: {}\noptimal steps: {}, ratio: {:.2f}'.format(
                    current_world, target_category, num_steps, success, minimum_optimal_steps, ratio_optimal_policy))
                # plt.show()
                ...
                plt.savefig('{}/env_{}_category_{}_id_{}_middle.jpg'.format(saved_folder,
                            current_world, target_category, idx), bbox_inches='tight')
                plt.close()
                ...
                plt.title('env: {}, target: {}, detection_score: {:.2f}'.format(
                    current_world, target_category, detection_score))
                plt.savefig('{}/env_{}_category_{}_id_{}_right.jpg'.format(saved_folder,
                            current_world, target_category, idx), bbox_inches='tight')
                plt.close()
            ...
            print('env: {}, category: {}, success rate: {}/{}, avg_ratio: {:.2f}'.format(current_world,
                  target_category, sum_success, len(list_init_img_id), avg_ratio))
